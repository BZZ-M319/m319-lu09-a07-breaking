...
def test_reaction_distance():
    assert reaction_distance(20) == pytest.approx(28.8)


def test_braking_distance_dry():
    assert braking_distance(20, True) == pytest.approx(57.14, 0.01)


def test_braking_distance_wet():
    assert braking_distance(20, False) == pytest.approx(100, 0.01)


def test_stopping_distance_dry():
    assert stopping_distance(72, True) == pytest.approx(85.94, 0.01)


def test_stopping_distance_wet():
    assert stopping_distance(72, False) == pytest.approx(128.8, 0.01)


def test_safety_distance_meter():
    assert safety_distance_meter(72, True) == pytest.approx(40)
